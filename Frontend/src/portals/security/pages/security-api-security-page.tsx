...
import { APISecurityMonitoringDashboard } from "../../../portals/developer/components/api-security-monitoring-dashboard";
import { logSecurityPortalActivity } from "../../../services/security-portal.service";

export function SecurityAPISecurityPage() {
  ...
  useEffect(() => {
    if (profile) {
      logSecurityPortalActivity({
        ...
        module: "API Security",
        action: "Accessed API Security Monitoring",
      }).catch(console.error);
    }
  }, [profile]);
  ...
  return (
    <div className="space-y-6">
      ...
      <APISecurityMonitoringDashboard />
    </div>
  );
}