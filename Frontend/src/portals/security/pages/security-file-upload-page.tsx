...
import { FileUploadSecurityMonitoringDashboard } from "../../../portals/developer/components/file-upload-security-monitoring-dashboard";
import { logSecurityPortalActivity } from "../../../services/security-portal.service";

export function SecurityFileUploadPage() {
  ...
  useEffect(() => {
    if (profile) {
      logSecurityPortalActivity({
        ...
        action: "Accessed File Upload Security Monitoring",
      }).catch(console.error);
    }
  }, [profile]);
  ...
  return (
    <div className="space-y-6">
      ...
      <FileUploadSecurityMonitoringDashboard />
    </div>
  );
}