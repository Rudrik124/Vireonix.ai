...
export function DeveloperErrorLogsPage() {
              ...
              {(['today', 'last7days', 'all'] as const).map((range) => (
                <button
                  ...
                  className={`px-4 py-2 rounded font-bold transition ${
                    timeRange === range ? 'bg-blue-600 text-white' : 'bg-slate-700 text-slate-400 hover:bg-slate-600'
                  }`}
                >
            ...
            <div className="flex gap-2 flex-wrap">
              {(['critical', 'high', 'medium', 'low'] as const).map((sev) => (
                <button
                  ...
                  onClick={() =>
                    setSeverityFilter(
                      severityFilter.includes(sev) ? severityFilter.filter((s) => s !== sev) : [...severityFilter, sev]
                    )
                  }
                  className={`px-4 py-2 rounded font-bold transition ${
                    severityFilter.includes(sev) ? getSeverityColor(sev) : 'bg-slate-700 text-slate-400 hover:bg-slate-600'
                  }`}
                >
            ...
            <div className="flex gap-2 flex-wrap">
              {(['open', 'resolved'] as const).map((stat) => (
                <button
                  key={stat}
                  onClick={() =>
                    setStatusFilter(statusFilter.includes(stat) ? statusFilter.filter((s) => s !== stat) : [...statusFilter, stat])
                  }
                  className={`px-4 py-2 rounded font-bold transition ${
                    statusFilter.includes(stat) ? getStatusColor(stat) : 'bg-slate-700 text-slate-400 hover:bg-slate-600'
                  }`}
                >
          ...
          <div className="flex gap-3 flex-wrap">
            {(['open', 'in-review', 'fixed', 'verified'] as const).map((status) => (
              <button
                ...
                disabled={isUpdating}
                className={`px-4 py-2 rounded font-bold transition ${
                  selectedBug.status === status ? 'bg-blue-600 text-white' : 'bg-slate-700 text-slate-400 hover:bg-slate-600'
                }`}
              >
                ...
                <button
                  ...
                  onClick={() =>
                    setBugSeverityFilter(
                      bugSeverityFilter.includes(sev) ? bugSeverityFilter.filter((s) => s !== sev) : [...bugSeverityFilter, sev]
                    )
                  }
                  className={`px-4 py-2 rounded font-bold transition ${
                    bugSeverityFilter.includes(sev) ? getSeverityColor(sev) : 'bg-slate-700 text-slate-400 hover:bg-slate-600'
                  }`}
                >
                  {sev.charAt(0).toUpperCase() + sev.slice(1)}
                </button>
            ...
            <div className="flex gap-2 flex-wrap">
              {(['open', 'in-review', 'fixed', 'verified'] as const).map((stat) => (
                <button
                  ...
                  onClick={() =>
                    setBugStatusFilter(
                      bugStatusFilter.includes(stat) ? bugStatusFilter.filter((s) => s !== stat) : [...bugStatusFilter, stat]
                    )
                  }
                  className={`px-4 py-2 rounded font-bold transition ${
                    bugStatusFilter.includes(stat) ? getStatusColor(stat) : 'bg-slate-700 text-slate-400 hover:bg-slate-600'
                  }`}
                >
  ...
  const renderTabContent = () => {
    if (activeTab === 'errors') {
      return selectedError ? renderErrorDetail() : renderErrorList();
    }

    return selectedBug ? renderBugDetail() : renderBugList();
  };
